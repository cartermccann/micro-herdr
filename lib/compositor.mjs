...
// `appid` carry the same identity, but Electron does not spell them the same
// way on every compositor: mango on atlas reported `grok-bot` / `cursor`,
// while Hyprland often surfaces the productName (`Grok Bot`, `Cursor`).
// Matching and wlrctl focus therefore go through lib/identity.mjs rather than
// comparing the raw string to profiles.json.
//
  ...
  const start = () => {
    ...
    sock.on("data", (chunk) => {
      ...
      for (const line of lines) {
        ...
        // Hyprland emits `activewindow>>,` when focus leaves every window.
        // Reporting that as null keeps the focused value live; dropping it left
        // a stale app_id behind, which a focus CHECK would then trust.
        onFocus({ appid: appid || null, title: appid ? title : null });
      }
    });
